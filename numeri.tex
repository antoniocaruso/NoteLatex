...
\centering \includegraphics[width=6cm,trim=1.3cm 2cm 1cm 1.2cm,clip]{graphics/abaco.jpg}
...
L'algoritmo pertanto è il seguente: \nota{Conversione tramite divisioni per potenze decrescenti della base} Se si vuole convertire il numero decimale $n$ in base $B$ si calcola $k = \log_B n$, si calcola $c_{k-1} = n / B^{k-1}$ e $n' = n \Mod{B^{k-1}}$, si diminuisce $k$ e si applica lo stesso metodo a $n'$.
...
\includegraphics[width=0.95\textwidth]{ascii-codes-table.png}
\caption{Tabella ASCII per la rappresentazione numerica dei caratteri (stampabili e non).}
\end{figure}
