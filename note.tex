...
\usepackage[a4paper,vmargin=30mm, 
            right=1.9in, marginparsep=3.5mm, footskip=15mm]{geometry}
\usepackage{epigraph} 
...
\newboxedtheorem[
	...
	titleboxcolor=black]{defn}{Definizione}{thCounter} 
...
Questa diversa prospettiva sarà utilissima per comprendere perchè scriviamo i
numeri in decimale, ed il legame stretto tra la scelta di una rappresentazione
numerica e gli \emph{algoritmi} necessari allo svolgimento delle operazioni
elementari. Oltre che per le necessità specifiche del corso di Programmazione,
questo Capitolo è sicuramente di interesse a chi dovrà svolgere attività
didattica nelle scuole elementari; evidenziando la natura essenzialmente
\emph{algoritmica} dell'Aritmetica.

Cominciamo quindi, \nota{rappresentazione vs.\\ valore} con il distingure
chiaramente un valore numerico, ad esempio l'intero $1234$, da una delle tante
possibili sue \emph{rappresentazioni}, termine che vuol indicare \emph{il modo
...
(intero) come una \emph{stringa}\footnote{sequenza di caratteri giustapposti}
di \emph{cifre} (\emph{digit} in inglese). Il numero di cifre usate è detto
\emph{base o radice} della rappresentazione, una definizione formale è fornita
di seguito:

%%%% Definizione Rappresentazione Posizionale 

\begin{defn}[Rappresentazione dei numeri Naturali in base $B$] Sia $B>1$ un intero che chiameremo \emph{base (o radice)} della rap\-presentazione, e $\mathcal{C}$ un insieme di $B$ simboli distinti.

Gli elementi di $\mathcal{C}$ sono dette \emph{cifre della rappresentazione}
(in inglese \emph{digit}). Ogni cifra è associata ad un valore intero tra $0$ e
$B-1$. Un numero naturale in base $B$ è scritto come una sequenza di cifre:
$(c_{k-1}c_{k-2} \ldots c_1c_0)_B$ ed il suo valore è dato dalla relazione:

\begin{equation}\label{eq:pos} 
	c_{k-1}B^{k-1}+c_{k-2}B^{k-2}+\ldots+c_1B+c_0 = \sum_{i=0}^{k-1} c_iB^i 
\end{equation}
\end{defn} %%%%

\noindent nel seguito se la base $B$ è chiara dal contesto non useremo le
parentesi e la base come pedice, pertanto $ (1001001)_2$ sarà scritto
semplicemente come $1001001$ se è chiaro che stiamo trattando numeri in base
$2$. Noi usiamo abitualmene la base $10$, poichè abbiamo $10$ dita, e questo
facilità il processo di apprendimento, sia della rappresentazione sia del
\emph{contare} in tale base. Abitualmente in base $10$ scriveremo \textsf{1298}
e non $\mbox{\textsf{(1298)}}_{10}$, ed il valore ad esso associato è l'intero
1298. Quest'ultima osservazione che può sembrare banale, è invece opportuna in
quanto è chiaro che data una sequenza di cifre, il valore ad essa associato
dipende dal sistema di numerazione in cui ci troviamo, cioè dalla base usata,
la stessa sequenza infatti avrà valori diversi a seconda della base.
...
1\cdot 2^1 = 1234$.\\ Ed $(\textsf{1234})_3$ ? Non ha senso valutare la
sequenza poichè se $B = 3$ si ha $\mathcal{C} = \{ 0, 1, 2 \}$ ed i simboli
\textsf{3,4} non hanno alcun significato in tale sistema.\end{ex}

Poichè in \eqref{eq:pos} le operazioni sono svolte in base $10$ essa fornisce
un modo per convertire un naturale dalla base $B \neq 10$ alla base $B = 10$,
negli esempi si vede come calcolare il valore (in base 10) di numeri binari o
ottali. La stessa formula ci mostra come i numeri siano strettamente collegati
al concetto di polinomio: la \eqref{eq:pos} può essere vista come un polinomio
di grado $k-1$, i cui coefficienti sono le cifre dell'intero, valutato nel
punto $x = B$.
...
\begin{ex} In base 10 con al più 4 cifre possiamo rappresentare i valori tra
$0$ e $9999$, questi corrispondono alle quadruple dell'insieme $\mathcal{C}^4 =
\left\{ (c_3,c_2,c_1,c_0) \;|\; 0 \leq c_i \in \mathbb{N} \leq 9 \right\} = \{
(0,0,0,0), \ldots, (9,9,9,9) \}$. Vi sono quindi $|\mathcal{C}^4| = 10000$
numeri possibili, da $0$ a $9999 = 10000-1$.
...
e $(11111111)_2 = 255$. In esadecimale con $4$ cifre abbiamo $16^4 = 65536$ valori possibili,
da $0$ a $\text{FFFF} = 65535$. \end{ex}
...
\lceil\, 8,0915 \,\rceil = 9$ cifre, poichè $\log_2{n} = 8,0915 / \log_{10}{2}
...
procedimento su $n'$ fin quando il valore della divisione è diverso da zero estraendo cosi tutte le cifre.

\begin{ex}
Consideriamo $n = 12345$, abbiamo $n' = n/10 = 1234$ e $n \Mod{10} = 5$. Ripetendo l'operazione di modulo su $n'$ otteniamo la cifra $4$, e cosi via. Se
...
\end{align*}
e poichè che l'ultima cifra è la più significativa si ha che $12345$ in binario è $11000000111001$. Proviamo a convertire $58$ in esadecimale, osserviamo che $58/16 = 3$ e $58 \Mod{16} = 10$ quindi $(58)_{10} = (3A)_{16}$. Convertiamo $12345$ in ottale: 
\begin{align*}
 12345 / 8 &= 1543 + 1 \\ 
 1543  / 8 &= 192 + 7   \\
 192  / 8  &= 24 + 0 \\
...
\]
\noindent Notiamo che si inizia a raggruppare dalle cifre \emph{meno} significative, cioè da destra a sinistra, nel caso in cui come nell'esempio si hanno meno cifre di quelle da raggruppare, si aggiungino degli zeri (in figura in grassetto) che posti davanti al numero non ne cambiamo il valore. Otteniamo
quindi esattamente il valore calcolato precedentemente. Lo stesso vale per la coversione in esadecimale, invece di usare le divisioni, poichè $16 = 2^4$ possiamo fare cosi:
\[
...
L'algoritmo pertanto è il seguente: Se si vuole convertire il numero decimale $n$ in base $B$ si calcola $k = \log_B n$, si calcola $c_{k-1} = n / B^{k-1}$ e $n' = n \Mod{B^{k-1}}$, si diminuisce $k$ e si riapplica il metodo a $n'$. 
\begin{ex}
...
Cominciamo con l'operazione più semplice, il calcolo del successore di un intero $n$, valutare cioè $n+1$, questo ci consentirà di imparare a contare in una generica base $B$. Il lettore svolge questa operazione in modo \emph{meccanico} poichè l'ha imparata in tal modo fin dalle elementari. 
...
\begin{ex}
Contiamo
 in binario: $\mathsf{0, 1, (1+{\mathrm 1}) = 10, (10+{\mathrm 1}) = 11, (11+{\mathrm 1}) = 100}$,\\ 
$\mathsf{100+{\mathrm 1} = 101, 101+{\mathrm 1} = 110}$, etc.\\
